...
public abstract class Entity<T> : EntityBase where T : Entity<T>
{
    ...
    {
        if (controller.enabled) HandlePosition();
    }
    ...
    protected virtual void HandleContacts()
    {
        ...
        for (var i = 0; i < overlaps; i++)
            ...
            if (!m_contactBuffer[i].isTrigger && m_contactBuffer[i].transform != transform)
            {
                ...
                // 依次调用对方的接触回调（实现双向交互）
                foreach (var contact in listeners) contact.OnEntityContact((T)this);
    ...
    {
        if (other != null) states.OnContact(other);
    }
    ...
    public virtual void FaceDirection(Vector3 direction, float degreesPerFrame)
    {
        if (direction.sqrMagnitude == 0) return;

        var currentRotation = transform.rotation;
        var rotationDelta = Time.deltaTime * degreesPerFrame;
        var targetRotation = Quaternion.LookRotation(direction, Vector3.up);
        //transform.rotation = Quaternion.Slerp(currentRotation, targetRotation, rotationDelta);
