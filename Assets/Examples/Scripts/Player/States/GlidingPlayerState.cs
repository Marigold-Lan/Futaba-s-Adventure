...
public class GlidingPlayerState : PlayerState
{
    ...
    protected override void OnStep(Player player)
    {
        ...
        else if (!player.inputs.GetGlide()) player.states.Change<FallPlayerState>();
    }
    ...
    protected virtual void HandleGlidingGravity(Player player)
    {
        ...
        var delta = player.stats.current.glidingGravity * Time.deltaTime * player.gravityMultiplier;
        var newYVelocity = yVelocity - delta;
        newYVelocity = Mathf.Max(newYVelocity, -player.stats.current.glidingMaxFallSpeed);
        player.verticalVelocity = new Vector3(0, newYVelocity, 0);
    }
}