...
{
    [Header("Parameters Names")] public string stateName = "State";
    ...
    [Header("Settings")] public float minLateralAnimationSpeed = 0.5f;
    ...
    protected virtual void InitializeForcedTransitions()
    {
        ...
        foreach (var forcedTransition in forcedTransitions)
            if (!m_forcedTransitions.ContainsKey(forcedTransition.fromStateId))
                m_forcedTransitions.Add(forcedTransition.fromStateId, forcedTransition);
    }
    ...
    public class ForcedTransition
    {
        ...
        [Tooltip("要强制播放的动画状态名")] public string toAnimationState;
    }
}