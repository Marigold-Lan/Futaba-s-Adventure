...
public class ItemBox : MonoBehaviour, IEntityContact
{
    ...
    protected float m_timeSinceLocked;

    protected float k_lockDuration = 0.1f;
    ...
    public virtual void Collect(Player player)
    {
        ...
        {
            // 还有未收集的物品
            if (m_index < collectables.Length && !m_locked)
            {
    ...
    protected void HandleLock()
    {
        if (m_locked)
        {
            m_timeSinceLocked +=  Time.deltaTime;
            if (m_timeSinceLocked >= k_lockDuration)
            {
                m_locked = false;
            }
        }
    ...
    public void OnEntityContact(EntityBase entity)
    {
        if (entity is Player player)
        {
            if (entity.velocity.y > 0 &&
                Mathf.Abs(entity.controller.bounds.max.y - m_collider.bounds.min.y) < 0.1f)
            {
                Collect(player);
            }
        }
