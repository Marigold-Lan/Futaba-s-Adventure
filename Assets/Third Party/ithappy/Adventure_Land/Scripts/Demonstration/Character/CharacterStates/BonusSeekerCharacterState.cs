    ...
    public class BonusSeekerCharacterState : CharacterStateBase
    {
        ...
        private void FindBonusItem()
        {
            ...
            for (int index = 0; index < hitColliders.Length; index++)
            {
                ...
                if (bonusItem != null && bonusItem.enabled && bonusItem.TryReserved() && IsPointReachable(bonusItem.transform.position, out _))
                {
        ...
        private bool GetRandomPointOnNavMesh()
        {
            ...
            if (!IsPointReachable(randomPoint, out var navMeshHit))
            {
                return false;
            }
        ...
        private bool IsPointReachable(Vector3 point, out NavMeshHit hit)
        {
