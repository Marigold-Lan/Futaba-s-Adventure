...
namespace ithappy.Adventure_Land
{
    ...
    public class ObstaclePathEditor : UnityEditor.Editor
    {
        ...
        private Color startPointColor = new Color(0.2f, 0.8f, 0.2f, 0.5f); // Зеленый
        private Color targetPointColor = new Color(0.8f, 0.2f, 0.2f, 0.5f); // Красный
        private Color lineColor = new Color(0.5f, 0.5f, 1f, 0.8f); // Синий
        ...
        private void HandlePointPlacement()
        {
            ...
            if (e.type == EventType.MouseDown && e.button == 0 && !e.alt && !e.control && !e.shift)
            {
                ...
                {
                    // Блокируем событие, чтобы оно не обрабатывалось другими системами
                    e.Use();
